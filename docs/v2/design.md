...
- **驗證輪修正（2026-09-10，S15 後）**：使用者裁決採「政策 B」——呼叫端對 🔴 逐條查證（重現失敗情境或讀碼確認）成立就直接修、不成立列「有異議」；🟡 仍等裁決；修正在提請 follow-up commit 時攤開，commit 閘門不變。另發現地圖修訂記錄能覆蓋 skill 流程（S10 寫的一筆讓 S11–S15 全部改手動審查），故明定「地圖與 skill 衝突以 skill 為準」，地圖的開場固定動作只記專案專屬事項
